/* ──────────────────────────────────────────────────────────────────────────
   OSINT-Spider-V2  • dependency-free • strict TS
   ------------------------------------------------------------------------ */
   ...
   /* cheap “stem”: lc→strip vowels→first4 */
   const cheapStem=(s:string)=>s.toLowerCase().split(/\W+/).map(w=>w.replace(/[aeiou]/g,"").slice(0,4)).join("");
   
   const ai=new OpenAI({ apiKey: OPENAI_API_KEY! });
   let usdSpent=0;
   
   async function llm(prompt:string,ctx:string,max:number):Promise<string>{
     const est=price(tokens(prompt)+tokens(ctx),max);
   ...
   const scoreSerp=(o:SerperOrganic,domain:string)=>{
     ...
     if(/invoice|contract|apikey|secret|password/i.test(o.snippet??"")) s+=0.2;
   ...
   export async function runSpider(raw:unknown):Promise<SpiderPayload>{
     ...
     /*──────── SERPER BFS ───────*/
     while(queue.length&&serper<MAX_SERPER&&performance.now()-begin<WALL_MS){
       const q=queue.shift()!;
       const res = await postJSON<SerperResp>(SERPER,{q,num:MAX_SERP_PAGE,gl:"us",hl:"en"},
         {"X-API-KEY":SERPER_KEY!}).catch(()=>({organic:[]}));
       serper++;
     ...
     for(let i=0;i<targets.length&&fire<MAX_FIRECRAWL;i+=BATCH_FIRE){
       await Promise.all(targets.slice(i,i+BATCH_FIRE).map(async h=>{
         if(performance.now()-begin>=WALL_MS) return;
     ...
     const addBullet=(sec:SectionName,text:string,src:number)=>{
       ...
       const sig=cheapStem(text).slice(0,240);
     ...
     const sections:Section[]=await Promise.all(SECTIONS.map(async name=>{
       ...
       const crit=bl.filter(b=>b.sev==="CRITICAL").length;
       const hi  =bl.filter(b=>b.sev==="HIGH").length;
       return {
         name,
         summary:await llm(
           `${bl.length} findings (${crit} CRITICAL, ${hi} HIGH). Summarize in ≤3 sentences; no new facts.`,
